...
internal class MethodCallHandlerImpl(context: Context, activity: Activity?) : MethodCallHandler {
								...
								?.apply {
									try {
										val data = hashMapOf<String, Any?>(
											...
											"dataActivity" to dataActivity,
											"radioType" to radioType(this),
											"cellId" to cellId(this, subsInfo.simSlotIndex),
											...
											"subscriptionId" to if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) subscriptionId else null,
											"isoCountryCode" to simCountryIso,
											"networkCountryIso" to if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) networkCountryIso else null,
											"mobileNetworkCode" to if (simOperator?.isEmpty() == false)
												simOperator?.substring(3) else null,
											"displayName" to simOperatorName,
											"mobileCountryCode" to if (simOperator?.isEmpty() == false)
												simOperator?.substring(0, 3) else null,
											"networkGeneration" to networkGeneration(this),
										)
										...
										Log.d(
											TAG,
											"TelephonyManager Exception: ${simOperatorName}\n${e}"
										)
	...
	private fun isMultiSimSupported(): String {
		if (mDefaultTelephonyManager?.hasCarrierPrivileges() == true) {
